...
update [PatientFlow].[Language] set [TranslationRefId]= 76 where [LanguageId]=1;
update [PatientFlow].[Language] set [TranslationRefId]= 77 where [LanguageId]=2;
update [PatientFlow].[Language] set [TranslationRefId]= 78 where [LanguageId]=3;
update [PatientFlow].[Language] set [TranslationRefId]= 79 where [LanguageId]=4;
update [PatientFlow].[Language] set [TranslationRefId]= 80 where [LanguageId]=5;
update [PatientFlow].[Language] set [TranslationRefId]= 81 where [LanguageId]=6;
go

update [PatientFlow].[OrganisationSystemType] set SystemType='EMIS - PCS' where SystemTypeId=2

insert [PatientFlow].[Report] ( [ReportName], [ReportOrder]) values ( N'Log Report', 1)
insert [PatientFlow].[Report] ( [ReportName], [ReportOrder]) values ( N'Questionnaire Report', 2)
insert [PatientFlow].[Report] ( [ReportName], [ReportOrder]) values ( N'Sync Service Report', 3)
set identity_insert [PatientFlow].[Report] off

delete from [PatientFlow].[SiteMenu] where MenuName='Diverts'
